...
## 四、P1 — ~~播放页迷你歌词~~ ❌ 已否决（2026-09-22）

> **结论：不移植。** 此方案曾实施后撤销，记录如下以免重犯。

### 4.1 LX-Pro 的实现

`PlayDetail/Vertical/index.tsx:88-96` + `components/MiniLyric.tsx`
- 封面页（第 0 页）底部悬浮当前歌词，位置 `bottom:6%`、左右 `10%`
- 取当前行 `text`（`:19-28`）+ 首条翻译 `extendedLyrics[0]`（`:26`），字号固定 `13`

### 4.2 ❌ 为什么本项目不该要它

| | LX-Pro | 本项目 |
|---|---|---|
| 封面与歌词 | **分两页**（`PagerView` 左右滑） | **同屏**（歌词左 + 封面右） |
| 看封面时 | 歌词**看不见** → 需要迷你歌词补位 | 歌词**就在左边**，一直可见 |
| 迷你歌词的作用 | **弥补分页缺陷的补丁** | **重复信息** |

> **判断失误记录**：我最初把它列为「值得移植的优点」，只看了 LX-Pro 有、本项目没有，
> **没想清楚它存在的理由**——它是为「分页」这个缺陷服务的。本项目同屏，
> 没有那个缺陷，照搬等于凭空多一行冗余信息。
>
> **教训**：移植前先问「这个设计**为什么存在**」，而不只是「它长什么样」。
> 若原设计是为弥补某个缺陷，而本项目没有该缺陷，则不应移植。

### 4.3 顺带修掉的真实问题

排查迷你歌词位置时，算出**原有布局本身有重叠**：

```
cover 底边 0.498  >  nowinfo 顶边 0.462      ❌ 重叠 -0.036（一直存在）
```
...
| `cover` | 0.39 | **0.335** |
| `nowinfo` | 0.485 | **0.470** |
| `transport` | 0.57 | **0.560** |

4 种屏幕比例验证无重叠（间隙 +0.040 ~ +0.062）。

### 4.4 歌词视觉规格（仍可参考）

LX-Pro 歌词是**两态**（`Vertical/Lyric.tsx:82-87`）：
```js
active   → [c-font / c-primary-font-active, c-primary-alpha-200, opacity 1]
inactive → [c-450, c-400, opacity 0.8]
```

> ⚠️ 本项目 `lyric3d` 已是**四态衰减**（距离 0/±1/±2/≥3 → 0.97/0.44/0.22/0.10），
> **更细腻，保留原样，不要照抄**。
...
> ~~原建议从 P1 开始~~ → P1 已否决，**改从 P0 开始**（统计图表，价值最大）。
> 注意：P0 需要新写 4 个绘制函数，工作量较大，建议逐个图表做、每个都跑测试。
